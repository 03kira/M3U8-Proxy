...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
    ...
    if (statusCode === 301 || statusCode === 302 || statusCode === 303 || statusCode === 307 || statusCode === 308) {
        ...
        let parsedLocation;
        if (locationHeader) {
            locationHeader = node_url_1.default.resolve(requestState.location.href, locationHeader);
...
function parseURL(req_url) {
    ...
    const parsed = node_url_1.default.parse(req_url);
    if (!parsed.hostname) {
...
function getHandler(options, proxy) {
    ...
    const hasRequiredHeaders = function (headers) {
        return (!corsAnywhere.requireHeader ||
    ...
    return function (req, res) {
        ...
        function isValidHostName(hostname) {
            ...
            return !!(regexp.test(hostname) || node_net_1.default.isIPv4(hostname) || node_net_1.default.isIPv6(hostname));
        }
...
function createRateLimitChecker(CORSANYWHERE_RATELIMIT) {
    ...
    let unlimitedPattern = rateLimitConfig[3]; // Will become a RegExp or void.
    if (unlimitedPattern) {
